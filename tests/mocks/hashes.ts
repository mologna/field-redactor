...
import { HASH_STRATEGIES, BinaryToTextEncoding, binaryToTextEncoding } from '../../src/types';
...
type CalculatedHashResult = Record<typeof binaryToTextEncoding[number], string>;
...
const hashGenerator = (text: string, algorithm: HASH_STRATEGIES) => {
  return binaryToTextEncoding.reduce((prev: Partial<CalculatedHashResult>, encoding: BinaryToTextEncoding) => {
    return {
      ...prev,
